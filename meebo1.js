...
function ClearVariableData(innings) {	//To consider how much this is needed.
...
function InitializeCenterBoard(innings) {
		...
		rowBatter[10].getElementsByTagName("td")[nReqdIdx].innerHTML = innings == 0? "" : (target+1); //FIX THE FUCKING TARGET
		rowBatter[13].getElementsByTagName("td")[ReqRateIdx].innerHTML = innings == 0? "" : "REQD RATE";
		rowBatter[13].getElementsByTagName("td")[nReqRateIdx].innerHTML = innings == 0? "" : "????";
}
...
function PlaySequence(looper) {
	innings = looper;
	//ClearVariableData(looper); //Need it or not?
	//SetIndexes(looper);
	SendOpeners(looper);
	
	//SetFooter(looper);
	InitializeCenterBoard(looper);
	Initialize();
