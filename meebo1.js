When openers initialized, Center header country
iff inn1 runs clear runs reqd, target
if inn1 runs clear reqd run rate
...
function InitializeCenterBoard(innings) {
	if (innings == 0) 
		batting_team = rowBatter[0].getElementsByTagName("td")[1] + "BATTING";
	else
		batting_team = rowBatter[0].getElementsByTagName("td")[8] + "BATTING";
	batting_team = rowBatter[0].getElementsByTagName("td")[7] = batting_team;

	if (innings == 0) {
		rowBatter[10].getElementsByTagName("td")[7].innerHTML = ""; 
		rowBatter[10].getElementsByTagName("td")[8].innerHTML = "";
		rowBatter[13].getElementsByTagName("td")[7].innerHTML = ""; 
		rowBatter[13].getElementsByTagName("td")[8].innerHTML = "";
	}
...
function WicketFallen(innings) {
	...
	if (innings == 1 && (str == 3 || str == 7)) {
		console.log("why the fuck are we hitting this?");
...
function PlaySequence(looper) {
	ClearVariableData(looper);
	SendOpeners(looper);
	Initialize();
	SetIndexes(looper);
	innings = looper;
...
function Do_Second_Object() {
	//timer = setInterval( function CallIt() { Changer("table2"); }, 500);
	timer = setInterval( function CallIt() { PlaySequence(1); }, 500);
}
		
function Do_Object(obj) {
	val = getRan(50,100);
	ref = document.getElementById(obj).getElementsByTagName("tr")[0];
	ref.getElementsByTagName("td")[0].innerHTML = val;
	count++;
...
function main() {
	...
	/*
	timer = setInterval( 
				//function CallIt() { Do_Object("table1"); }, 
