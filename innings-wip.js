...
function main() {
	...
	var batsmen = []; 
    for (idx = 0; idx < 2; idx++) {
		batsmen.push({ name: '', howout : '', runs : 0, faced : 0, fours : 0, sixes : 0 });
		batsmen[idx]['name'] = name[idx];
		
		ChangeRowColor(rowBatter, idx, "#32ff7e");
		rowBatter[idx].getElementsByTagName("td")[0].innerHTML = batsmen[idx]['name']; 
		rowBatter[idx].getElementsByTagName("td")[1].innerHTML = "batting"; 
		rowBatter[idx].getElementsByTagName("td")[2].innerHTML = batsmen[idx]['runs']; 
		rowBatter[idx].getElementsByTagName("td")[3].innerHTML = batsmen[idx]['faced']; 
    }
    ...
    function doInning() {
        ...
        if (ball_result == 9) {   //WICKET
			...
			if (wkts == 10)
				...
				rowBatter[idx].getElementsByTagName("td")[0].innerHTML = batsmen[str]['name']; 
				rowBatter[idx].getElementsByTagName("td")[1].innerHTML = "batting"; 
				rowBatter[idx].getElementsByTagName("td")[2].innerHTML = 0; 
				rowBatter[idx].getElementsByTagName("td")[3].innerHTML = 0; 
				//rowBatter[idx].getElementsByTagName("td")[2].innerHTML = batsmen[str]['runs']; 
				//rowBatter[idx].getElementsByTagName("td")[3].innerHTML = batsmen[str]['faced']; 
			}
        }
